...
fn main() {
    ...
    for i in 0..20 {
        for j in 0..20 {
            if i + 3 < 20 {
                result = max(result, matrix[i][j] * matrix[i + 1][j] * matrix[i + 2][j] * matrix[i + 3][j]);
                if j > 2 {
                    result = max(result, matrix[i][j] * matrix[i + 1][j - 1] * matrix[i + 2][j - 2] * matrix[i + 3][j - 3]);
                }
            }

            if j + 3 < 20 {
                result = max(result, matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2] * matrix[i][j + 3]);
                if i > 2 {
                    result = max(result, matrix[i][j] * matrix[i - 1][j + 1] * matrix[i - 2][j + 2] * matrix[i - 3][j + 3]);
                }
            }
